...
sns.lineplot(x='episode', y='return_ratio', hue='planning_horizon', data=df, palette='colorblind', ci=None)
#sns.despine(trim=True)

number_of_trials = df.groupby(['planning_horizon', 'trial_number']).count().groupby('planning_horizon').count().iloc[0,0]


plt.title('Planning agent (on empty policy) in 7-blocks world')
plt.xlabel('Episode')
